...
class ResNetManifoldMixup(nn.Module):
    """
    This wraps a ResNet model so that Manifold Mixup can be applied inside intermediate layers.
...
def build_model(num_classes: int = 14, backbone_name: str = "resnet18", alpha: float = 2.0):
    ...
    model = ResNetManifoldMixup(backbone, alpha=alpha)
    return model
